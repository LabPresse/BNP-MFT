%% HMC posterior sampler for sample 1D trajectory based the binned time traces (2DG-Cylindrical), and is modified for implicit diffusion
function [ q_new , rec ] = HMC_binned_time_trace_Cylinder( ...
    ...
q_old    , N        , phi   , V0    , ...
Y        , w        , mu    , I_b   , ...
I_mol    , rec      , demo  , h     , J )
...
Vq = get_V_BS_grad_Cylinder( q_mid(:,j) , w , I_b , I_mo );
...
log_a = get_V_BS_Cylinder(q_mid(:,1)    , w , I_b , I_mo ) - ...
        get_V_BS_Cylinder(q_mid(:,end) , w , I_b , I_mo ) ...
...
%% HMC potential gradient (likelihood only) of the binned time traces (2D Gaussian-Cylindrical)
function Vq = get_V_BS_grad_Cylinder(q , w , I_b , I_mol)
...
%% HMC potential (likelihood only) of the binned time traces (2D Gaussian-Cylindrical)
function V = get_V_BS_Cylinder(q , w , I_b , I_mol)
