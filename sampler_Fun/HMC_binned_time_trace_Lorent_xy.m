%% HMC posterior sampler for sample 1D trajectory based the binned time traces (2D-Lorentzian-XY coordinates), and is modified for implicit diffusion
function [q_new,rec] = HMC_binned_time_trace_Lorent_xy( ...
    ...
q_old    , N      , phi   , V0    , ...
Y        , w      , mu    , I_b   , ...
Z        , I_mol  , rec   , demo  , ...
...
Vq = get_V_BS_grad_Lorent_xy( q_mid(:,j) , Y , Z , w , I_b , I_mol );
...
log_a = get_V_BS_Lorent_xy(q_mid(:,1)   , Y , Z , w ,  I_b ,I_mol ) - ...
        get_V_BS_Lorent_xy(q_mid(:,end) , Y , Z , w , I_b ,I_mol ) ...
...
%% HMC potential gradient (likelihood only) of the binned time traces (2D Gaussian-Lorentzian in X and Y coordinates)
function Vq = get_V_BS_grad_Lorent_xy(q , Y , Z , w , I_b , I_mol)

         mm       = I_mol.*exp(-2*(((q').^2)+(Y.^2))./(1+(Z.^2)))./(1+(Z.^2)) ;   
         mu       = I_b+mm                                                    ;
         mu_prim  = -4*q'.*mm./(1+(Z.^2))                                     ;
...
%% HMC potential (likelihood only) of the binned time traces (2D Gaussian-Lorentzian in X and Y coordinates)
function V = get_V_BS_Lorent_xy(q,Y,Z,w,I_b,I_mol)


         mu  = I_b+I_mol.*exp(-2*(((q').^2)+(Y.^2))./(1+(Z.^2)))./(1+(Z.^2)) ;
